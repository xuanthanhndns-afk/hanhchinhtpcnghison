...
function buildRangeReport(db, startDate, endDate, department = "") {
  ...
  const dayKeys = Array.from(
    new Set([
      ...orders.map((o) => o.mealDate),
      ...db.menus.filter((m) => m.mealDate >= startDate && m.mealDate <= endDate).map((m) => m.mealDate),
    ])
  ).sort();
...
async function api(req, res) {
  ...
  try {
    ...
    if (route === "GET /api/reports/range") {
      ...
      return json(res, 200, buildRangeReport(db, startDate, endDate, department));
    }
